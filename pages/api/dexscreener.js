...
export default async function handler(req, res) {
  ...
  try {
    // Use the token-pairs endpoint (returns an array of pools)
    ...
    const tokenSide =
      pair.baseToken.address === address ? pair.baseToken : pair.quoteToken;
    ...
    // Buy Score (% buys vs sells in 24h)
    let buyScore = "N/A";
    const tx24 = pair.txns?.h24;
    if (tx24 && tx24.buys + tx24.sells > 0) {
      buyScore = Math.round(
        (tx24.buys / (tx24.buys + tx24.sells)) * 100
      ).toString();
    }
    ...
    for (const label of pair.labels || []) {
      ...
      if (label === "freezable")
        warnings.push("Freeze authority not renounced");
      if (label === "honeypot") warnings.push("Possible honeypot");
    }

    if (typeof pair.priceChange?.h24 === "number") {
      if (pair.priceChange.h24 < -50)
        warnings.push("Down >50% in 24h");
      if (pair.priceChange.h24 > 1000)
        warnings.push("Up >1000% in 24h");
    }
